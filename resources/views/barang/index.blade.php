    ...
    <div class="py-6 px-4">
        ...
        <form method="GET" action="{{ route('barang.index') }}" class="mb-4 flex flex-wrap gap-2">
            ...
            @if (request('cari') || request('rak_id'))
                <a href="{{ route('barang.index') }}" class="text-sm text-red-600 underline">Reset</a>
            @endif
        </form>
        ...
        <div class="bg-white p-4 rounded shadow">
            <table class="min-w-full bg-white border">
                ...
                <tbody>
                    @forelse ($barangs as $index => $barang)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border px-4 py-2">
                                <a href="{{ route('barang.edit', $barang->id) }}" class="bg-yellow-400 text-white px-2 py-1 rounded text-sm">Edit</a>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-4 text-gray-500">Barang tidak ditemukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
